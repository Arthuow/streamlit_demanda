    ...
    for mes in range (1,13):
         if mes in df_filtrado_ano['MES'].values:
            ...
            for selecao in df_atributos['Cód. do Trafo/Alimentador']:
                ...
                valor_minimo_S = min(df_base_filtrado['S'])
                valor_minimo_S = round(float(valor_minimo_S),2)
                valores_sem_zero = [valor for valor in df_base_filtrado['P'] if valor != 0]

                if valores_sem_zero:
                    # Calculando o valor mínimo excluindo zero
                    valor_minimo_P_sem_zero = min(valores_sem_zero)
                else:
                    # Se não houver valores diferentes de zero, atribuir None ou algum outro valor de acordo com sua lógica
                    valor_minimo_P_sem_zero = None
                valor_minimo_Q = min(df_base_filtrado['Q'])
                ...
                base_filtrada_minimo = np.where(df_base_filtrado['P'] > (valor_media_p - 3.0 * desvio_padrao_P),
                                                df_base_filtrado['P'], np.nan)
                valor_minimo_filtrado = np.nanmin(base_filtrada_minimo)

                if valor_minimo_P_sem_zero / minimo_desvio > 0.9:
                    valor_minimo_P = valor_minimo_filtrado
                else:
                    valor_minimo_P = valor_minimo_P_sem_zero

                print("Valor Mínimo: ", valor_minimo_P)

                # Obtenha a lista de índices onde 'P' é igual ao valor mínimo
                min_date_time_index = df_base_filtrado.index[df_base_filtrado['P'] == valor_minimo_P].tolist()
                ...
                if min_date_time_index and not pd.isna(min_date_time_index[0]):
                    ...
                    #indice_valor_minimo_P = df_base_filtrado['P'].idxmin()
                    #valor_Q_correspondente = df_base_filtrado.at[indice_valor_minimo_P,'Q']
                    #print(f"Valor de 'Q' AJUSTADO correspondente ao mínimo de 'P': {valor_Q_correspondente}")


                potencia_aparente_mínima = np.sqrt((valor_minimo_P_sem_zero ** 2) + (valor_Q_correspondente ** 2))

                ################################### POTENCIA ATIVA ################################################
                if valor_minimo_P_sem_zero is not None:  # Verifica se um valor válido foi calculado
                    linha = df_atributos[df_atributos['Cód. do Trafo/Alimentador'] == selecao].index[0]
                    coluna_mes_ano = f'{meses[mes - 1]} {ano}'
                    df_dados_com_meses_anos.loc[linha, coluna_mes_ano] = valor_minimo_P_sem_zero
                ...
                ################################# POTENCIA APARENTE ####################################################
                if valor_minimo_S is not None:  # Verifica se um valor válido foi calculado
                    linha = df_atributos[df_atributos['Cód. do Trafo/Alimentador'] == selecao].index[0]
                    coluna_mes_ano = f'{meses[mes - 1]} {ano}'
                    df_dados_com_meses_anos_S.loc[linha, coluna_mes_ano] = potencia_aparente_mínima
                #######################################################################################################
...
################################## INSERINDO A INFORMAÇÃO DE POTENCIA ATIVA ###############################################
...
df_dados_com_meses_anos['Pot. Mínima'] = df_dados_com_meses_anos.drop(excluir_colunas, axis=1).min(axis=1).round(2)
...
df_dados_com_meses_anos['Carregamento'] = (df_dados_com_meses_anos['Pot. Mínima'] / df_dados_com_meses_anos['Potencia Instalada'].where(df_dados_com_meses_anos['Potencia Instalada'] > 0, other=1))*100
...
################################# INSERINDO A INFORMAÇÃO DE POTENCIA REATIVA ###########################################

# Converta a coluna 'Potencia Instalada' para valores numéricos
df_dados_com_meses_anos_Q['Potencia Instalada'] = pd.to_numeric(df_dados_com_meses_anos_Q['Potencia Instalada'], errors='coerce')
excluir_colunas= ['Descrição', 'Cód. de Ident', 'Cód. do Trafo/Alimentador', 'Potencia Instalada','Tensão Prim','Tensão Sec. (kV)']

df_dados_com_meses_anos_Q['Pot. Reativa'] = df_dados_com_meses_anos_Q.drop(excluir_colunas, axis=1).min(axis=1).round(2)
# Divide o máximo de cada linha pela coluna 'Potencia Instalada' se for maior que zero
...
df_dados_com_meses_anos_S['Pot. Aparente'] = df_dados_com_meses_anos_S.drop(excluir_colunas, axis=1).min(axis=1).round(2)
# Divide o máximo de cada linha pela coluna 'Potencia Instalada' se for maior que zero
...
dir = r"C:\Users\Engeselt\Documents\GitHub\ASPO_2\Demanda_Mínima_Não_Coincidente.xlsx"

# Primeira aba
with pd.ExcelWriter(dir, engine='openpyxl', mode='w') as writer:
    df_dados_com_meses_anos.to_excel(writer, sheet_name="Potência Ativa Mínima", index=False)

# Segunda aba
with pd.ExcelWriter(dir, engine='openpyxl', mode='a') as writer:
    df_dados_com_meses_anos_Q.to_excel(writer, sheet_name="Potência Reativa Mínima", index=False)
# Terceira aba
with pd.ExcelWriter(dir, engine='openpyxl', mode='a') as writer:
    df_dados_com_meses_anos_S.to_excel(writer, sheet_name="Potência Aparente Mínima", index=False)
