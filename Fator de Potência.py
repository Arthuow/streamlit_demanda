...
qtd_leituras_fora_faixa = 0
qtd_leituras_dentro_faixa = 0
max_date_time = None
resultados = []
################################################################
...
df_dados_com_meses_anos_Q_médio= pd.concat([df_dados_tecnicos, df_meses_anos], axis=1)
    ...
    for mes in range (1,13):
         if mes in df_filtrado_ano['MES'].values:
            ...
            for selecao in df_atributos['Cód. do Trafo/Alimentador']:
                ...
                df_base_filtrado['fp - médio'] = df_base_filtrado['fp'].mean()
                ...
                df_base_filtrado["Ultrapassagem"] = df_base_filtrado.apply(lambda row: row["ultrapassagem"].sum(),
                                                                           axis=1)
                horas_sobrecarga = df_base_filtrado["Ultrapassagem"].sum()

                ######################### FATOR DE POTENCIA ############################

                df_base_filtrado['fp - condicao'] = pd.cut(df_base_filtrado['fp'],
                                                           bins=[-float('inf'), 0.92, float('inf')],
                                                           labels=['abaixo de 0.92', 'acima de 0.92'])
                contagem = df_base_filtrado['fp - condicao'].value_counts().to_dict()
                contagem_abaixo_092 = contagem.get('abaixo de 0.92', 0)
                contagem_acima_092 = contagem.get('acima de 0.92', 0)

                # Filtrar o DataFrame para incluir apenas os valores abaixo de 0.92
                df_abaixo_092 = df_base_filtrado[df_base_filtrado['fp'] < 0.92]
                df_acima_092 = df_base_filtrado[df_base_filtrado['fp'] > 0.92]

                # Calcular a média do fator de potência abaixo de 0.92
                media_fp_abaixo_092 = df_abaixo_092['fp'].mean()
                media_fp_acima_092 = df_acima_092['fp'].mean()

                ############## CORREÇÃO DE FATOR DE POTÊNCIA ###########################
                #Sfinal = P/f.p (0.98)
                df_base_filtrado['S_corrigido'] = df_base_filtrado['P'] / 0.98
                angulo_graus = 11.8
                angulo_radianos = math.radians(angulo_graus)
                seno = math.sin(angulo_radianos)
                df_base_filtrado['Q_correção'] = df_base_filtrado['Q'] - (df_base_filtrado['S_corrigido'] * seno)
                # Calcule a média da coluna Q_correção
                Q_correcao_media = df_base_filtrado['Q_correção'].mean()
                # Q final = S final *sen(11,5)
                # Qcorreçaõ = Qmedido - Qfinal
                valor_media_Q = np.mean(df_base_filtrado['Q'])
                valor_fp_médio = np.mean(df_base_filtrado['fp - médio'])
                resultados.append({
                    'Cód. do Trafo/Alimentador': [selecao],
                    'Ano': [ano],
                    'Mês': [mes],
                    'Contagem_abaixo_0.92': [contagem_abaixo_092],
                    'Contagem_acima_0.92': [contagem_acima_092],
                    'Média_fp_abaixo_092': [media_fp_abaixo_092],
                    'Média_fp_acima_092': [media_fp_acima_092],
               ...
               # print("desvio Padrão: ", desvio_padrao_P)
                ...
                ################################# POTENCIA REATIVA MÉDIA ################################################
                if valor_Q_correspondente is not None:  # Verifica se um valor válido foi calculado
                    linha = df_atributos[df_atributos['Cód. do Trafo/Alimentador'] == selecao].index[0]
                    coluna_mes_ano = f'{meses[mes - 1]} {ano}'
                    df_dados_com_meses_anos_Q_médio.loc[linha, coluna_mes_ano] =valor_media_Q
                #######################################################################################################
...
df_contagem = pd.DataFrame(resultados)
df_contagem['Cód. do Trafo/Alimentador'] = df_contagem['Cód. do Trafo/Alimentador'].str[0]
df_contagem['Ano'] = df_contagem['Ano'].apply(lambda x: x[0])
df_contagem['Mês'] = df_contagem['Mês'].apply(lambda x: x[0])
df_contagem['Contagem_abaixo_0.92'] = df_contagem['Contagem_abaixo_0.92'].apply(lambda x: x[0])
df_contagem['Contagem_acima_0.92'] = df_contagem['Contagem_acima_0.92'].apply(lambda x: x[0])
df_contagem['Média_fp_abaixo_092'] = df_contagem['Média_fp_abaixo_092'].apply(lambda x: x[0])
df_contagem['Média_fp_acima_092'] = df_contagem['Média_fp_acima_092'].apply(lambda x: x[0])
df_contagem['BC necessário'] = df_contagem['BC necessário'].apply(lambda x: x[0])
df_contagem['valor_fp_médio'] = df_contagem['valor_fp_médio'].apply(lambda x: x[0])
df_contagem['Horas Sobrecarga'] = df_contagem['Horas Sobrecarga'].apply(lambda x: x[0])
...
df_numerico = df_dados_com_meses_anos.drop(excluir_colunas, axis=1).apply(pd.to_numeric, errors='coerce')
df_dados_com_meses_anos['Pot. Máxima'] = df_numerico.max(axis=1).round(2)
...
df_numerico = df_dados_com_meses_anos_Q.drop(excluir_colunas, axis=1).apply(pd.to_numeric, errors='coerce')
df_dados_com_meses_anos_Q['Pot. Reativa'] = df_numerico.max(axis=1).round(2)


#df_dados_com_meses_anos_Q['Pot. Reativa'] = df_dados_com_meses_anos_Q.drop(excluir_colunas, axis=1).max(axis=1).round(2)

df_numerico = df_dados_com_meses_anos_Q_médio.drop(excluir_colunas, axis=1).apply(pd.to_numeric, errors='coerce')
df_dados_com_meses_anos_Q_médio['Pot. Reativa Média'] = df_numerico.mean(axis=1).round(2)




#df_dados_com_meses_anos_Q_médio['Pot. Reativa Média'] = df_dados_com_meses_anos_Q_médio.drop(excluir_colunas, axis=1).mean(axis=1).round(2)
# Divide o máximo de cada linha pela coluna 'Potencia Instalada' se for maior que zero

#COLUNAS NÃO NUMERICAS
non_numeric_columns = df_dados_com_meses_anos_Q.select_dtypes(exclude=[np.number]).columns
#print("Colunas não numéricas:", non_numeric_columns)
#df_dados_com_meses_anos_Q['Carregamento'] = (df_dados_com_meses_anos_S['Pot. Aparente'] / df_dados_com_meses_anos_Q['Potencia Instalada'].where(df_dados_com_meses_anos_Q['Potencia Instalada'] > 0, other=1))*100
#df_dados_com_meses_anos_Q['Carregamento'] =df_dados_com_meses_anos_S['Carregamento'].round(2)
df_dados_com_meses_anos_Q.set_index('Cód. do Trafo/Alimentador')
#df_dados_com_meses_anos_Q["Ultrapassagem"] = sum(df_base_filtrado['ultrapassagem'])
...
df_numerico = df_dados_com_meses_anos_S.drop(excluir_colunas, axis=1).apply(pd.to_numeric, errors='coerce')
df_dados_com_meses_anos_S['Pot. Aparente'] = df_numerico.max(axis=1).round(2)


#df_dados_com_meses_anos_S['Pot. Aparente'] = df_dados_com_meses_anos_S.drop(excluir_colunas, axis=1).max(axis=1).round(2)
# Divide o máximo de cada linha pela coluna 'Potencia Instalada' se for maior que zero
...
df_dados_com_meses_anos_S['Carregamento'] =df_dados_com_meses_anos_S['Carregamento'].round(2)
...
dir = r"C:\Users\Engeselt\Documents\GitHub\ASPO_2\Fator de Potência.xlsx"
...
# Segunda aba
with pd.ExcelWriter(dir, mode='a', engine='openpyxl') as writer:
...
with pd.ExcelWriter(dir, engine='openpyxl', mode='a') as writer:
    df_dados_com_meses_anos_Q_médio.to_excel(writer, sheet_name="Potência Reativa Média", index=False)
# Quarta aba
with pd.ExcelWriter(dir, engine='openpyxl', mode='a') as writer:
    df_dados_com_meses_anos_S.to_excel(writer, sheet_name="Potência Aparente", index=False)

with pd.ExcelWriter(dir, mode='a', engine='openpyxl') as writer:
    df_contagem.to_excel(writer, sheet_name='Fator de Potência', index=False)
