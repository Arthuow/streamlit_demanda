...
url_base_original = r"C:\Users\Engeselt\Documents\GitHub\ASPO_2\Medição Agrupada.csv"
df_base_original = pd.read_csv(url_base_original, sep=";", encoding='latin-1')
...
df_base['SEMANA'] =  df_base.index.isocalendar().week
df_base['MES'] = df_base['MES'].astype(int)
df_base['ANO'] = df_base['ANO'].astype(int)
df_base['SEMANA'] = df_base['SEMANA'].astype(int)
print("\nImportação do df_base Concluída")
print(df_base)

print('Importanto tadela informativa:\n')
...
##########DADOS TéCNICOS################
print('Importanto tadela informativa para Dados Técnicos\n')
url_atributos = r"C:\Users\Engeselt\Documents\GitHub\ASPO_2\Tabela informativa.xlsx"
...
mes=1
anos = range(2025,2025)
meses = ['Janeiro','Fevereiro','Março','Abril','Maio','Junho','Julho','Agosto','Setembro','Outubro','Novembro','Dezembro']
semana = range(1,53)


#Criação da Tabela com os anos, meses e semanas
colunas_meses_anos = []
colunas_meses_anos = [f'Semana {semana} {mes} {ano}' for ano in anos for mes in meses for semana in range(1, 53)]
print(colunas_meses_anos)
...
df_valores_maximo_P = pd.DataFrame(columns = ['Cód. do Trafo/Alimentador'] + colunas_meses_anos)
    ...
    # Supondo que df_base já esteja definido e contém as colunas 'ANO' e 'MES'
data_atual = datetime.datetime.now()
semana_atual = data_atual.isocalendar()[1]
...
for ano in range(2025, 2026):
    ...
    for mes in range(1, 13):
        ...
        if mes in df_filtrado_ano['MES'].values:
            ...
            # Calcular o número máximo da semana para o mês
            ...
            print("Tipo de dados de semanas:", semanas.dtype)
            print("Semanas:", semanas)
            # Verifique se semanas não está vazio usando len()
            if len(semanas) > 0:
                max_semana = semanas.max()  # Número máximo da semana
                ...
                for semana in range(1, max_semana + 1):
                    ...
                    if not df_filtrado_semana.empty:  # Verifique se há registros para a semana
                        ...
                        ########################
                            ...
                            #####################################################################################################
                            ###################################### BASE FILTRADA ######
                            df_base_filtrado = pd.DataFrame(df_filtrado_semana, columns=['DATA_HORA',descricao, descricao_saida, descricao_Q, descricao_saida_Q])
                            ...
                            df_base_filtrado["Ultrapassagem"] = df_base_filtrado.apply(lambda row: row["ultrapassagem"].sum(),axis=1)
                            ...
                            ################################# POTENCIA REATIVA #####################################################
                            if valor_media_Q is not None:  # Verifica se um valor válido foi calculado
                                linha = df_atributos[df_atributos['Cód. do Trafo/Alimentador'] == selecao].index[0]
                                coluna_mes_ano = f'Semana {semana} {meses[mes - 1]} {ano}'
...
################################## INSERINDO A INFORMAÇÃO DA ABA POTENCIA REATIVA #######################################
# Converta a coluna 'Potencia Instalada' para valores numéricos
...
dir = r"C:\Users\Engeselt\Documents\GitHub\ASPO_2\Demanda_Máxima_Semana.xlsx"
...
# Segunda aba
#df_nova_aba = pd.DataFrame({'Coluna1': [1, 2, 3], 'Coluna2': ['A', 'B', 'C']})
